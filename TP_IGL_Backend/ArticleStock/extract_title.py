...
def download_file_from_drive(pdf_url, output_path):
    file_id = pdf_url.split('/')[-2]
    pdf_direct_url = f'https://drive.google.com/uc?id={file_id}'
    gdown.download(pdf_direct_url, output_path, quiet=False)

def pdf_title_from_drive(file_id):
    tmp_path = 'temp.pdf'
    download_file_from_drive(file_id, tmp_path)
    title = pdf_title(tmp_path)
    return title
