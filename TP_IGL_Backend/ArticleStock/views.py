...
def retrieve_all_data(request):
    ...
    s = Search(index='articles_igl2').query('match_all')
...
def retrieve_validated_data(request):
    ...
    s = Search(index='articles_igl2').query('bool', filter=Q('term', validated=True))
    s = s.extra(size=1000)  # Change 1000 to the desired number of hits
...
def search_articles(request):
    try:
        ...
        s = Search(index='articles_igl2').query(query)
...
def retrieve_latest_validated_articles(request):
    try:
        # Create a search instance
        s = Search(index='articles_igl2').query(Q('match_all') & Q('term', validated=True))
